...
def _load_tasks(path: Path) -> list[dict[str, Any]]:
	...
	raise ValueError("Expected a Label Studio export as a list of tasks or a wrapped dict")
...
def _boundary_issues(source_text: str, start: int, end: int, span_text: str) -> list[str]:
	issues: list[str] = []
		...
		for result in results:
			...
			for issue in issues:
				yield Issue(
					...
					issue=issue,
					span_text=span_text,
					source_text=source_text[start:end] if 0 <= start <= end <= len(source_text) else "",
				)
...
def main(argv: Optional[list[str]] = None) -> int:
	...
	for issue in issues:
		print(
			f"task={issue.task_id} annotation={issue.annotation_id} result={issue.result_id} "
